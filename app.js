...
// Informational reasons carry context a reviewer should see without holding the record.
// They are rendered separately so "Further review required" keeps meaning what it says,
// and so a record whose only reason is context still shows it.
function reviewSectionHtml(review) {
  const items = review.reasons || [];
  const holds = items.filter(reason => !reason.informational);
  const context = items.filter(reason => reason.informational);
  const list = reasons => `<ul>${reasons.map(reason => `<li>${escapeHtml(reason.message)}${reason.evidence ? `<span class="secondary-line">${escapeHtml(formatPriorityText(reason.evidence))}</span>` : ""}</li>`).join("")}</ul>`;
  const blocks = [];
  if (holds.length) blocks.push(`<section class="review-notice" aria-label="Further review required"><h3>Further review required</h3>${list(holds)}<p>Review these points alongside the shown patch priority.</p></section>`);
  if (context.length) blocks.push(`<section class="review-context" aria-label="Context"><h3>Context</h3>${list(context)}</section>`);
  return blocks.join("");
}
...
function renderDetail() {
  ...
  $("detail-panel").innerHTML = `
    ...
    ${reviewSectionHtml(record.review)}
...
// Seed the view from the fragment synchronously, before anything async runs.
// loadText resyncs the hash once a month is loaded, and it finishes first; with
// state.view still on its default it rewrote the fragment to the landing view
// and a deep link to the advisory list was lost before applyHash ever read it.
state.view = readHashView();
