...
2. **Read `definitions` in the supplied taxonomy before assigning any judgement tag.** Tag names are not self-explanatory and several have a different meaning elsewhere in security. `user-content` in particular means attacker-supplied content a user must open - not content authored by users.
2b. Select tags only from the supplied taxonomy and mitigation IDs only from the supplied mitigation catalogue.
...
3a. **`remediation_context` states only whether a vendor fix exists and any documented workaround.** Update availability per product is read from the vendor's structured remediation list, not from your prose, so do not narrate it. Never describe KB links or release-note references as though they were remediations other than patching - in CVRF, "remediation" means the patch.
3b. **Keep every factor to one or two plain sentences.** State what the source says. No hedging, no restating the CVSS vector in words, no "it should be noted that". A factor longer than roughly 40 words is too long.

4. Copy the exact CVSS basis into `cvss_basis`. The merge rejects any mismatch with Microsoft data.
5. Every judgement tag is an object `{"tag": ..., "evidence": ...}`. The evidence must be a short fragment **quoted or closely paraphrased from the supplied record** - the title, a product name, the vendor notes or FAQ. A tag you cannot cite is a tag you should not assert. Citing your own reasoning rather than the source is a failed citation.
5a. Assert a workload tag whenever the supplied record names the affected role or component - "Windows DNS Server", "Services for NFS", "Kerberos", "Remote Desktop", "SMB Server", "Windows Shell". Naming the component is exactly the evidence rule 5 asks for, so there is no reason to omit it. Abstaining is correct only when the record does not identify a role at all.
