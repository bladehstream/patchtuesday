...
**Do not use the CISA-ADP CVSS for Chromium, for anything.** Verified 2026-09-12 by direct fetch: CVE-2026-84353 (Chromium **Critical**), CVE-2026-84354 (**High**) and CVE-2026-85047 (**Medium**) all carry ADP 9.6 on the *identical* vector `AV:N/AC:L/PR:N/UI:R/S:C/C:H/I:H/A:H`. All three descriptions contain "execute arbitrary code outside the sandbox", so the enrichment appears to key on outcome wording rather than on the bug. The inverse also occurs: CVE-2026-85052 is Chromium **High** but ADP **3.1 Low** — an out-of-bounds read by an already-compromised renderer, stage two of an escape chain. Filtering or sorting on that score drops it. CISA's own SSVC on the same record says Technical Impact `partial`, which is more honest than its own CVSS. Carry the ADP score only as a labelled, non-sortable annotation, or not at all; use SSVC Technical Impact instead.

**The Chromium tier is an impact scale and nothing else.** `chromium/src/docs/security/severity-guidelines.md` mentions CVSS zero times. Critical means "read or write arbitrary resources on the underlying platform, with the user's full privileges" — outside the sandbox. High means "execute code in the context of, or otherwise impersonate other origins" — inside the sandbox, or stage two of a chain. Pairing the Chromium tier with a CVSS base score measures impact twice and exploitability never. **Google's own service levels are 30 days to all users for Critical and 60 for High**, so a Chromium Critical is not "drop everything" even to Google — do not let it drive an emergency action. A use-after-free marked `MiraclePtr Status: PROTECTED` is not treated as a security issue by Google at all.

**Two axes, two fields, never one blended score.** Impact is the Chromium tier or SSVC Technical Impact. Exploitation is CISA KEV first, then the Chrome Releases "Google is aware that an exploit for CVE-… exists in the wild" wording, then SSVC Exploitation, then EPSS — and EPSS sorts within a tier, it never sets one. Emit a **compliance window**, fixed by Essential Eight for a high-risk application at the client's maturity level and therefore auditable rather than a judgement, separately from an **operational priority** of impact against exploitation, which is where the analyst value sits.
